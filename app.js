...
const sample = { signs: [], land: [{deed:'12345',name:'ที่ดินโครงการ Lakefront',area:2450,use:'พาณิชยกรรม',appraisal:245000000,tax:735000,status:'ตรวจสอบแล้ว'},{deed:'67890',name:'อาคารสำนักงาน',area:840,use:'อื่น ๆ',appraisal:125000000,tax:375000,status:'ชำระแล้ว'}] };
...
function connectFirebase(){
  try {
    ...
    cloudDb=firebase.firestore(); const record=cloudDb.collection('taxData').doc('current');
    record.onSnapshot(async snapshot=>{
      ...
      else { cloudReady=true; await record.set(db); syncMessage('สร้างข้อมูลกลางแล้ว'); }
...
function render(){
  ...
  $('#landRows').innerHTML=db.land.map(x=>`<tr><td><b>${x.deed}</b></td><td>${x.name}</td><td>${Number(x.area).toLocaleString()} ตร.ว.</td><td>${x.use}</td><td>${fmt(x.appraisal)}</td><td><b>${fmt(x.tax)}</b></td><td><span class="badge ${statusClass(x.status)}">${x.status}</span></td><td><button class="row-action" onclick="openEdit('land','${x.deed}')">แก้ไข</button></td></tr>`).join('');
...
function openForm(t,x=null){ type=t; editing=x; $('#modalTitle').textContent=(x?'แก้ไข':'เพิ่ม')+(t==='sign'?'รายการป้าย':'รายการที่ดิน'); $('#formFields').innerHTML=t==='sign'?signFields(x||{}):landFields(x||{}); if(t==='sign'){ ['#categorySelect','#widthInput','#lengthInput','#qtyInput'].forEach(s=>$(s).addEventListener('input',refreshCalculation)); $('#imagesInput').addEventListener('change',previewImages); } $('#recordModal').showModal(); }
async function compressImage(file){ return new Promise((resolve,reject)=>{ const reader=new FileReader(); reader.onload=()=>{const img=new Image();img.onload=()=>{const max=900, scale=Math.min(1,max/Math.max(img.width,img.height)), canvas=document.createElement('canvas');canvas.width=Math.round(img.width*scale);canvas.height=Math.round(img.height*scale);canvas.getContext('2d').drawImage(img,0,0,canvas.width,canvas.height);resolve(canvas.toDataURL('image/jpeg',.72));};img.onerror=reject;img.src=reader.result;};reader.onerror=reject;reader.readAsDataURL(file); }); }
